    ...
    <div class="main">
      <div class="title" style="justify-content: space-between">
        <div>
          ...
          <el-select v-model="ctxData.queryParams.dictType" placeholder="字典名称" clearable style="width: 200px;margin-right: 10px" @change="handleQuery">
            <el-option
          ...
          <el-input
            v-model="ctxData.queryParams.dictLabel"
            placeholder="请输入字典标签"
            clearable
            @change="handleQuery"
            style="width: 200px;"
    ...
    <el-dialog :title="ctxData.title" v-model="ctxData.open" width="500px" append-to-body :close-on-click-modal="false">
      <el-form ref="formRef" :model="ctxData.form" :rules="ctxData.rules" label-width="80px">
        <el-form-item label="字典类型">
          <el-input v-model="ctxData.form.dictType" :disabled="true" />
        </el-form-item>
        <el-form-item label="数据标签" prop="dictLabel">
          <el-input v-model="ctxData.form.dictLabel" placeholder="请输入数据标签"></el-input>
        </el-form-item>
        <el-form-item label="数据键值" prop="dictValue">
          <el-input v-model="ctxData.form.dictValue" placeholder="请输入数据键值"></el-input>
        </el-form-item>
        <el-form-item label="显示排序" prop="dictSort">
          <el-input-number v-model="ctxData.form.dictSort" controls-position="right" :min="0" />
        </el-form-item>
...
// 查询字典类型详细
const getType = (dictId) => {
  const pdata = {
    token: users.token,
    data: {dictId: Number(dictId)}
  }
  typeApi.getType(pdata).then(response => {
    ctxData.queryParams.dictType = response.data.dictType
    ctxData.defaultDictType = response.data.dictType
    getList()
  })
}
...
const getTypeList = () => {
  ...
  typeApi.listType(pdata).then(response => {
    ctxData.typeOptions = response.data.data
  })
}
...
const getList = () => {
  ...
  dataApi.listData(pdata).then(response => {
    ctxData.dataList = response.data.data
...
const resetQuery = () => {
  ctxData.queryParams = {
    dictType: ctxData.defaultDictType,
    pageNum: 1,
    pageSize: 10,
  }
  handleQuery()
...
const handleAdd = () => {
  reset()
  ctxData.open = true
  ctxData.title = '添加字典数据'
  ctxData.form.dictType = ctxData.queryParams.dictType
}
...
const handleUpdate = (row) => {
  ...
  dataApi.getData(pdata).then(response => {
...
const submitForm = () => {
  formRef.value.validate(valid => {
    if (valid) {
      ...
      // eslint-disable-next-line eqeqeq
      if (ctxData.form.dictCode != undefined) {
        dataApi.updateData(pdata).then(response => {
          ElMessage({type: 'success', message: '修改成功',})
          ctxData.open = false
          getList()
        })
      } else {
        dataApi.addData(pdata).then(response => {
...
const handleDelete = (row) => {
  const dictCode = row.dictCode || this.ids
  ElMessageBox.confirm('是否确认删除字典编码为"' + dictCode + '"的数据项?', '警告', {
