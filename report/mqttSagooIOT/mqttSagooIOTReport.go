...
type MQTTSagooIOTReportPropertyTemplate struct {
	DeviceType string //设备类型，"gw" "node"
	DeviceName []string
}

type MQTTSagooIOTPropertyPostParamPropertyTemplate struct {
	Name      string      `json:"name"`
	Value     interface{} `json:"value"`
	TimeStamp int64       `json:"timestamp"`
}

type MQTTSagooIOTReportAlarmTemplate struct {
	DeviceType string //设备类型，"gw" "node"
	DeviceName []string
	Properties []MQTTSagooIOTPropertyPostParamPropertyTemplate
}

type MQTTSagooIOTPropertyPostParamTemplate struct {
	DeviceCode string                                          `json:"devCode"`
	OnLine     bool                                            `json:"onLine"`
	TypeCode   string                                          `json:"typeCode"`
	VarData    []MQTTSagooIOTPropertyPostParamPropertyTemplate `json:"varData"`
}

type MQTTSagooIOTPropertyPostTemplate struct {
	ClientID string                                  `json:"clientID"`
	Params   []MQTTSagooIOTPropertyPostParamTemplate `json:"params"`
}

type MQTTSagooIOTReportPropertyAckTemplate struct {
...
func (r *ReportServiceParamSagooIOTTemplate) MQTTSagooIOTPropertyPost(rType int, gwParam ReportServiceGWParamSagooIOTTemplate, propertyParam []MQTTSagooIOTPropertyPostParamTemplate, alarm bool) (int, bool) {

	propertyPost := MQTTSagooIOTPropertyPostTemplate{
	...
	MQTTSagooIOTAddCommunicationMessage(r, propertyPostTopic, Direction_TX, string(sJson))

	if gwParam.MQTTClient != nil {
		token := gwParam.MQTTClient.Publish(propertyPostTopic, 1, false, sJson)
		if token.WaitTimeout(time.Duration(SagooIOTTimeOutReportProperty)*time.Second) == false {
			if alarm == false {
				setting.ZAPS.Errorf("SagooIOT上报服务[%s]发布上报属性消息失败 %v", gwParam.ServiceName, token.Error())
			} else {
				setting.ZAPS.Errorf("SagooIOT上报服务[%s]发布上报告警消息失败 %v", gwParam.ServiceName, token.Error())
			}
			return MsgID, false
		}
	} else {
		if alarm == false {
			setting.ZAPS.Errorf("SagooIOT上报服务[%s]发布上报属性消息失败", gwParam.ServiceName)
		} else {
			setting.ZAPS.Errorf("SagooIOT上报服务[%s]发布上报告警消息失败", gwParam.ServiceName)
		}
		return MsgID, false
	}

	if alarm == false {
		setting.ZAPS.Debugf("SagooIOT上报服务[%s]发布上报属性消息成功", gwParam.ServiceName)
	} else {
		setting.ZAPS.Debugf("SagooIOT上报服务[%s]发布上报告警消息成功", gwParam.ServiceName)
	}
	return MsgID, true
}

func (r *ReportServiceParamSagooIOTTemplate) GWPropertyPost() {

	propertyMap := make([]MQTTSagooIOTPropertyPostParamPropertyTemplate, 0)
	...
	//
	//property.Name = "SystemSagooIOTC"
	//property.Value = setting.SystemState.SystemSagooIOTC
	//property.TimeStamp = timeStamp
	//propertyMap = append(propertyMap, property)
	//
	...
	_, rt := r.MQTTSagooIOTPropertyPost(DeviceTypeGW, r.GWParam, propertyPostParamMap, false)
	if rt == true {
...
//指定设备上传属性
func (r *ReportServiceParamSagooIOTTemplate) NodePropertyPost(name []string, properties []MQTTSagooIOTPropertyPostParamPropertyTemplate, alarm bool) {
		...
		for k, v := range r.NodeList {
			if n == v.Name {
				...
				if alarm == true {
					...
					propertyPostParamMap = append(propertyPostParamMap, propertyPostParam)
					r.MQTTSagooIOTPropertyPost(DeviceTypeNode, r.GWParam, propertyPostParamMap[len(propertyPostParamMap)-1:], alarm)
				} else {
					property := MQTTSagooIOTPropertyPostParamPropertyTemplate{}
