...
type MQTTSagooIOTLogInDataTemplate struct {
	ProductKey string `json:"productKey,omitempty"`
	DeviceName string `json:"deviceName,omitempty"`
}

type MQTTSagooIOTLogInAckTemplate struct {
	ID      string                          `json:"id"`
	Code    int32                           `json:"code"`
	Message string                          `json:"message"`
	Data    []MQTTSagooIOTLogInDataTemplate `json:"data,omitempty"`
}

var MsgID int = 0

func (r *ReportServiceParamSagooIOTTemplate) MQTTSagooIOTOnConnectHandler(client MQTT.Client) {

	setting.ZAPS.Debug("MQTTSagooIOT 链接成功")
	r.GWParam.ReportStatus = "onLine"
	MQTTSagooIOTAddCommunicationMessage(r, "MQTTSagooIOT 链接成功", Direction_RX, "")
	...
	MQTTSagooIOTSubscribeTopic(r.GWParam.ServiceName, r.GWParam.MQTTClient, subTopic)
...
func (r *ReportServiceParamSagooIOTTemplate) MQTTSagooIOTConnectionLostHandler(client MQTT.Client, err error) {

	MQTTSagooIOTAddCommunicationMessage(r, "MQTTSagooIOT 链接断开", Direction_RX, "")
	setting.LedOnOff(setting.LEDServer, setting.LEDOFF)
	setting.ZAPS.Infof("MQTTSagooIOT 上报服务名称[%v]链接断开 %v", r.GWParam.ServiceName, err)
	r.GWParam.ReportStatus = "offLine"
}

func MQTTSagooIOTGWLogin(service *ReportServiceParamSagooIOTTemplate, publishHandler MQTT.MessageHandler) (bool, *MQTT.ClientOptions, MQTT.Client) {
	...
	opts.SetOnConnectHandler(service.MQTTSagooIOTOnConnectHandler)
	opts.SetConnectionLostHandler(service.MQTTSagooIOTConnectionLostHandler)
	opts.SetConnectRetry(true)
	opts.SetConnectRetryInterval(30 * time.Second)
	opts.SetConnectTimeout(time.Duration(SagooIOTTimeOutLogin) * time.Second)
	opts.SetAutoReconnect(true)

	mqttClient := MQTT.NewClient(opts)

	setting.ZAPS.Infof("上报服务[%s]网关登录", service.GWParam.ServiceName)
	MQTTSagooIOTAddCommunicationMessage(service, "网关登录", Direction_TX, "")
	...
	//MQTTSagooIOTAddCommunicationMessage(service, "网关登录成功", Direction_RX, nil)

	return true, opts, mqttClient
}

func MQTTSagooIOTSubscribeTopic(serviceName string, client MQTT.Client, topic string) {

	token := client.Subscribe(topic, 1, nil)
	if token.WaitTimeout(time.Duration(SagooIOTTimeOutSubscribe)*time.Second) == false {
		setting.ZAPS.Errorf("SagooIOT上报服务[%s]订阅主题[%s]失败 %v", serviceName, topic, token.Error())
		return
	}
	setting.ZAPS.Infof("SagooIOT上报服务[%s]订阅主题[%s]成功", serviceName, topic)
}

func (r *ReportServiceParamSagooIOTTemplate) GWLogin() bool {
	status := false
	status, r.GWParam.MQTTClientOptions, r.GWParam.MQTTClient = MQTTSagooIOTGWLogin(r, ReceiveMessageHandler)

	return status
}

func MQTTSagooIOTNodeLoginIn(param ReportServiceGWParamSagooIOTTemplate, nodeMap []string) int {
	...
	if len(nodeLogin.Params) > 0 {
		...
		if param.MQTTClient != nil {
			...
			case <-token.Done():
				{
					if token.Error() != nil {
						setting.ZAPS.Errorf("SagooIOT上报服务[%s]发布节点上线消息失败 %v", param.ServiceName, token.Error())
						return MsgID
					}
				}
			case <-time.After(time.Duration(SagooIOTTimeOutLogin) * time.Second):
				{
					setting.ZAPS.Errorf("SagooIOT上报服务[%s]发布节点上线消息失败 %v", param.ServiceName, token.Error())
					return MsgID
				}
	...
	setting.ZAPS.Infof("SagooIOT上报服务[%s]发布节点上线消息成功", param.ServiceName)

	return MsgID
}

func (r *ReportServiceParamSagooIOTTemplate) NodeLogIn(name string) error {
