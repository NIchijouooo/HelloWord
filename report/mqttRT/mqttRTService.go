...
type MQTTRTInvokeServiceAckTemplate struct {
	Code       int                    `json:"code"`
...
type MQTTRTInvokeServiceRequestTemplate struct {
	ClientID   string                 `json:"clientID"`
	MsgID      string                 `json:"msgID"`
	DeviceCode string                 `json:"devCode"`
	CmdName    string                 `json:"cmdName"`
...
func (r *ReportServiceParamRTTemplate) ReportServiceRTInvokeServiceAck(reqFrame MQTTRTInvokeServiceRequestTemplate, deviceType int, ackParam MQTTRTInvokeServiceAckTemplate) {
	...
	setting.ZAPS.Infof("RT上报服务发布回复写自定义命令应答消息主题 %s", serviceInvokeTopic)
	setting.ZAPS.Debugf("RT上报服务发布回复写自定义命令应答消息内容 %v", string(sJson))
	MQTTRTAddCommunicationMessage(r, "MQTT写自定义命令应答包", Direction_TX, string(sJson))

	if r.GWParam.MQTTClient != nil {
		token := r.GWParam.MQTTClient.Publish(serviceInvokeTopic, 1, false, sJson)
		if token.WaitTimeout(time.Duration(RTTimeOutService)*time.Second) == false {
			setting.ZAPS.Errorf("RT上报服务[%s]发布回复服务调用消息失败 %v", r.GWParam.ServiceName, token.Error())
			return
		}
	} else {
		setting.ZAPS.Errorf("RT上报服务[%s]发布回复服务调用消息失败", r.GWParam.ServiceName)
		return
	}
	setting.ZAPS.Debugf("RT上报服务[%s]发布回复服务调用消息成功", r.GWParam.ServiceName)
}

func (r *ReportServiceParamRTTemplate) ReportServiceRTProcessInvokeService(reqFrame MQTTRTInvokeServiceRequestTemplate) {

	ackParam := MQTTRTInvokeServiceAckTemplate{
	...
	MQTTRTAddCommunicationMessage(r, "MQTT写自定义命令请求包", Direction_RX, string(rData))

	//命令是针对网关的命令
	if reqFrame.DeviceCode == r.GWParam.Param.ClientID {
		ackParam.Code = r.ReportServiceRTProcessInvokeGWService(reqFrame)
		//r.ReportServiceRTInvokeServiceAck(reqFrame, DeviceTypeNode, ackParam)
	} else {
		//命令是针对设备的命令
...
func (r *ReportServiceParamRTTemplate) ReportServiceRTProcessInvokeGWService(reqFrame MQTTRTInvokeServiceRequestTemplate) int {
				...
				case <-reader.Finish:
					{
						...
						if index > 0 {
							...
							_ = r.ReportServiceRTGWOTA(taskID, reqFrame.ClientID, 100, 0)
						}
						return 0
					}
...
func (r *ReportServiceParamRTTemplate) ReportServiceRTGWOTA(taskId string, clientID string, percent int, code int) error {
