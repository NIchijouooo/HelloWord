	...
	case logrus.ErrorLevel:
		{
			...
			entry.Data["1timeStamp"] = time.Now().Local().Format("01-02T15:04:05.000")
			entry.Data["2fileLine"] = fmt.Sprintf("%05d", line)
			entry.Data["3goRoutineID"] = getGoRoutineID()
		}
	}
...
func (lg *LoggerParamTemplate) ReadParamFromJson() bool {
	...
	fileDir := exeCurDir + "/selfpara/loggerParam.json"
	...
	case 1: //1表示同时输出到终端和文件
		{
			...
			*/
			...
			creatTime := time.Now().Local().Format("20060102")
			file, _ := rotatelogs.New(
				LoggerParam.FileDir+creatTime+".log",
