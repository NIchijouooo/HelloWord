...
func LuaInit() {
	...
	if err := L.DoFile(path + "td200.lua"); err != nil {
		ZAPS.Warnf("open td200.lua fail %v", err)
	}
	ZAPS.Info("open TD200.lua OK")
...
func GetCRCModbus(L *lua.LState) int {
	...
	if err := gluamapper.Map(lv, &LuaVariableMap); err != nil {
		ZAPS.Warnf("GetCRC16 gluamapper.Map err %v", err)
	}
 ...
 * CRC-16/XMODEM
 */
func crc16Xmodem(buf []byte) uint16 {
	var crc uint16 = 0
	var ch byte = 0
	...
	for i := 0; i < size; i++ {
		ch = byte(crc >> 12)
		crc <<= 4
		crc ^= crc16TabXmodem[ch^(buf[i]/16)]

		ch = byte(crc >> 12)
		crc <<= 4
		crc ^= crc16TabXmodem[ch^(buf[i]&0x0f)]
	}
	return crc
}

func GetCRCXmodem(L *lua.LState) int {
...
func GoLog(L *lua.LState) int {
	...
	ZAPS.Debugf("Lua调试输出[%s]", string(nBytes))
